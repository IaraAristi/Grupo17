...
CREATE OR ALTER PROCEDURE club.InsertarPasesPileta 
AS
BEGIN
    SET NOCOUNT ON;
    ...
    INSERT INTO club.pasePileta (tipo, fechaDesde, fechaHasta, idSocio, codCostoPileta)
    SELECT 
		p.tipo,
        p.fechaDesde,
        p.fechaHasta,
...
BEGIN
    ...
    FROM tesoreria.detalleFactura df
    JOIN club.pasePileta p ON p.idSocio = df.ID_socio
    CROSS APPLY (
