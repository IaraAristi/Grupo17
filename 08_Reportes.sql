...
BEGIN
    ...
    JOIN club.catSocio cs ON s.codCat = cs.codCat
    JOIN club.actDeportiva ad ON ia.act = ad.codAct
...
BEGIN
    ...
    FROM socio.socio s
    JOIN club.catSocio cs ON s.codCat = cs.codCat
    JOIN club.Presentismo p ON s.ID_socio = p.socio
    JOIN club.actDeportiva ad ON p.act = ad.codAct
